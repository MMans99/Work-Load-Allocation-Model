...
def view():
    if current_user.role == Role.ADMIN.value:
        ...
        ranks_count = db.session.query(Rank).count()
        profiles_count = db.session.query(Profile).count()
        departments_count = db.session.query(Department).count()
        courses_count = db.session.query(Course).count()
        return render_template("dashboard/admin.html", users_count=users_count, ranks_count=ranks_count, profiles_count=profiles_count, departments_count=departments_count, courses_count=courses_count)
    ...
    total = sumdt+sumgs+sumr+sumuc+suml

    esumdt = current_user.profile.direct_teaching
    esumgs = current_user.profile.supervision
    esumr = current_user.profile.research
    esumuc = current_user.profile.university_service
    esuml = current_user.profile.lead_manage_admin
    etotal = esumdt+esumgs+esumr+esumuc+esuml

    return render_template("dashboard/user.html", sumdt=sumdt, sumgs=sumgs, sumr=sumr, sumuc=sumuc, suml=suml, total=total, esumdt=esumdt, esumgs=esumgs, esumr=esumr, esumuc=esumuc, esuml=esuml, etotal=etotal)
